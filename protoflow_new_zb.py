...
def create_flow_table_from_zb_packet(packet):
    ...
    zb_sa_2byte_node = None
    zb_da_2byte_node = None

    zb_sa_8byte = None

    #wpan is the address of the network within the channel
    #ntwkaddr is the address of a node within the network
    try:
    ...
    try:
        zb_sa_2byte_node = packet.zbee_nwk.src
    except:
        pass

    try:
        zb_da_2byte_node = packet.zbee_nwk.dst
    except:
        pass
    ...
    if (zb_sa_2byte, zb_sa_2byte_node) not in zb_two_byte_dict:
        zb_two_byte_dict[(zb_sa_2byte, zb_sa_2byte_node)] = zb_sa_8byte

    # zb_sa_8byte = packet.wpan.src64 if zb_sa_8byte is None else zb_sa_8byte
    # if zb_sa_2byte not in zb_two_byte_dict:
    #     zb_two_byte_dict[(zb_sa_2byte, zb_sa_2byte_node)] = zb_sa_8byte
    if zb_sa_8byte is None:
        return

    sa_oui = zb_sa_2byte if zb_sa_8byte is None else zb_sa_8byte[0:11]
    da_oui = zb_da_2byte if (zb_da_2byte, zb_da_2byte_node) not in zb_two_byte_dict else zb_two_byte_dict[(zb_da_2byte, zb_da_2byte_node)][0:11]
    
    zb_sa = zb_sa_2byte if zb_sa_8byte is None else zb_sa_8byte
    zb_da = zb_da_2byte if (zb_da_2byte, zb_da_2byte_node) not in zb_two_byte_dict else zb_two_byte_dict[(zb_da_2byte, zb_da_2byte_node)]
